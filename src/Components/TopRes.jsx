...
const TopRes = ({ header, subHeader, imageId, name, avgRating, slaString, cuisines, areaName, size }) => {
  const { cdn } = useGlobalContext()

  return (
    <div className={`${size==='lg' ? "":""} font-medium`}>
      <div className={`relative cursor-pointer ${size === "sm" ? "w-[300px] hover:scale-90 transition-transform duration-200" : "w-[250px] transition-transform duration-200 hover:scale-105"}`}>
    ...
    </svg> &nbsp; {avgRating} • {slaString}
        </p>
        <p className='text-sm text-gray-400'>
          {cuisines.join(", ").length > 30 ? cuisines.join(", ").slice(0, 30) + "..." : cuisines.join(", ")}
        </p>
